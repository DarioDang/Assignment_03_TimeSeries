...
**If the point is above the line:** We fail to reject the null hypothesis that there is no autocorrelation in the residuals.

**If the point is below the line:** we reject the null hypothesis that there is no autocorrelation in the residuals, indicating that there is significant autocorrelation in the residuals at these lags.
...
```{r}
# Fit the MA(1) model to time series C
...
### A. Identify a suitable ARMA for Series A
...
**Ljung - Box p - values:** According to the p values for Ljung - Box statistic, most of p-values at certain lags (1,3,5,7,9,10) fall below the 0.05 significant level. This means that we reject the null hypothesis that there is no autocorrelation in the residuals, indicating that there is significant autocorrelation in the residuals at these lags.
...
### B. Identify a suitable ARMA for Series B
...
The PACF of the residuals shows a lag very close to the line of significance at lag = 8.  However, given we are limited to p = 0, 1, and 2 the purposes of the assignment, it does not appear that adding an AR component with these low p values would benefit the model.

As adding an AR component does not appear beneficial, Let's now try subtracting one from the MA component to make an ARMA(0,0) white noise model to see if it will still lead to a good fit.
...
The ACF plot shows significance at lag 1 indicating the original ARMA(0,1) model was the right choice. Furthermore, all the p-values for the Ljung-Box statistic are below 0, which means that we can reject the null hypothesis that there is no significant autocorrelation for the residuals for these lags. This all points to ARMA(0,0) being a poor model for this time series. The original ARMA(0,1) model is likely a good fit. Increasing the MA component by one to an ARMA(0,2) model may lead to overfitting, considering the residuals for ARMA(0,1) already approximate white noise. It is better to keep the simpler model here.

#### C.5 Try Auto Arima for series C
...
auto_arima_seriesC
```
`auto.arima()` provides an ARMA(2,4) model.
...
series_C_aic_values <- data.frame(
  Model = c("ARMA(0,0)","ARMA(0,1)","ARMA(1,1)", "ARMA(1,2)", "ARMA(2,1)", "ARMA(2,2)", "Auto_Arima"),
  AIC = c(aic_arma00_seriesC,aic_arma01_seriesC,aic_arma11_seriesC, aic_arma12_seriesC, aic_arma21_seriesC,
...
### D. Identify a suitable ARMA for Series D
...
#### D.5 Try Auto Arima For Comparison
...
### Extracting Time Series and plotting it

Use the library quantmod to extract the daily time series of the share prices of the
Burberry Group by using the command
`getSymbols("BB2.BE",from="2015-01-01",to="2024-08-31")`, extract the adjusted
price and plot it.
...
Two series `bb2_adjusted` will be extracted. It will then be differenced to form `bb2_adjusted_diff`.
...
::: boxTask
Give a brief explanation of the drop in stock prices as reported by the news.
:::
...
::: boxTask
What is an adequate ARMA model for the adjusted price? Produce a tsdiag for
the chosen model with comment.
:::
...
The AR($1$) model appears to be a decent fit: 
  ...
  (iii) the p values for the Ljung-Box statistic are all above the line of significance, showing that we can fail to reject the null hypothesis that there is autocorrelation in the residuals. The AIC value is $2213.18$. 
...
Having run the above `summary` and `tsdiag`s of AR($0$) to AR($3$), it appears that ARMA($1$, $0$) = AR($1$) fits `bb2_adjusted` the best. The coefficients are **TODO**
...
::: boxTask
Use the xts library to transoform the daily adjusted prices from the previous
question to a monthly close series and plot it.
:::
...
::: boxTask
STAT456 and ECON614 students should also comment on the different models
for the daily series and the monthly series.
:::
