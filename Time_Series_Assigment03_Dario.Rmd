...
For Series A, all possible ARMA(p,q) models with $0 \leq p \leq 2$ and $0 \leq q \leq 2$ will be tried, and the most suitable model from this selection will ultimately be decided. For Series B, C, and C a different, more selective decision process will be used.

#### A.1 Try with ARMA(1,0) model

From Question 1, an AR(1) = ARMA(1,0) model may be a good fit, so let's begin here. Note that the PACF and ACF plots show noticeable patterns indicating that it is not just white noise, so an ARMA(0,0) model will not be tried. 

```{r}
# Fit the model with AR(1)
arma_10_model_seriesA <- Arima(data$Series.A, order = c(1, 0, 0))

# Evaluate the model 
tsdiag(arma_10_model_seriesA)
```
...
As a result, ARMA(1,0) may not fully capture the underlying autocorrelation structure of the time series indicate this is not a best fit model for this series.

#### A.2 Try with ARMA(2,0) model

```{r}
# Fit the model with ARMA(2,0)
arma_20_model_seriesA <- Arima(data$Series.A, order = c(2, 0, 0))

# Evaluate the model 
tsdiag(arma_20_model_seriesA)
```
...
arma_11_model_seriesA <- Arima(data$Series.A, order = c(1, 0, 1))

# Evaluate the model 
tsdiag(arma_11_model_seriesA)
```

**Standardized Residuals:** The residuals are centered around zero with no visible patterns, which indicates that the ARMA(1,1) model has effectively captured the major structure of the data.
...
Therefore, ARMA(1,1) is not an adequate model for this time series since not capture all pattern of data.

#### A.4 Try with ARMA(1,2) model

```{r}
# Fit the model with AR(1,2)
arma_12_model_seriesA <- Arima(data$Series.A, order = c(1, 0, 2))

# Evaluate the model 
tsdiag(arma_12_model_seriesA)
```
...
Therefore, ARMA(1,2) is represented as an adequate model for this series.

#### A.5 Try with ARMA(0,1) model

```{r}
# Fit the model with AR(0,1)
...
The ARMA(0,1) model is not a good fit for this time series, as there is still significant autocorrelation in the residuals which the model cannot capture.

#### A.6 Try with ARMA(0,2) model

```{r}
# Fit the model with AR(0,2)
arma_02_model_seriesA <- Arima(data$Series.A, order = c(0, 0, 2))

# Evaluate the model 
tsdiag(arma_02_model_seriesA)
```
...
The ARMA(0,2) model is not a good fit for this time series, as there is still significant autocorrelation in the residuals which the model cannot capture.

#### A.7 Try with ARMA(2,1) model

```{r}
# Fit the model with AR(2,1)
...
This suggest that the ARMA(2,1) is an adequate model for this series but the p values at lag 9 just slightly above the line, hence, ARMA(2,0,2) might get a better result in this case.

#### A.8 Try with ARMA(2,2) model

```{r}
# Fit the model with AR(2,2)
...
As a result, ARMA(2,2) seem to be an adequate model for this series.
...
aic_arma10_seriesA <- arma_10_model_seriesA$aic 
aic_arma20_seriesA <- arma_20_model_seriesA$aic 
aic_arma11_seriesA <- arma_11_model_seriesA$aic
aic_arma12_seriesA <- arma_12_model_seriesA$aic
aic_arma01_seriesA <- arma_01_model_seriesA$aic
aic_arma02_seriesA <- arma_02_model_seriesA$aic
aic_arma21_seriesA <- arma_21_model_seriesA$aic
aic_arma22_seriesA <- arma_22_model_seriesA$aic
```

```{r}
# Create a data frame for the AIC values
aic_values <- data.frame(
  Model = c("ARMA(1,0)", "ARMA(2,0)", "ARMA(1,1)", "ARMA(1,2)", 
            "ARMA(0,1)", "ARMA(0,2)", "ARMA(2,1)", "ARMA(2,2)"),
  AIC = c(aic_arma10_seriesA, aic_arma20_seriesA, aic_arma11_seriesA, 
          aic_arma12_seriesA, aic_arma01_seriesA, aic_arma02_seriesA,
          aic_arma21_seriesA, aic_arma22_seriesA)
)
...
We used AIC to compared different ARIMA model, based on the table ARIMA(2,2) have the lowest AIC and based on the tsdiag shown below, the model has captured all of the autocorrelation pattern and leave the white noise on the residual. Hence, it perform as an adequate model for this particular time series.

In order to interpret the model we extract the coefficient of the ARMA(2,2)

```{r}
# Extract the coefficients of ARMA(2,2)
summary(arma_22_model_seriesA)
```

The ARMA(2,2) model for the time series indicates that the current value is influenced by both past values and past forecast errors. The AR(1) (-1.6181) and AR(2) (-0.6846) terms suggest an inverse relationship with the values at lag 1 and lag 2, meaning that if the previous values were high, the current value is likely to be lower. The MA(1) (0.8915) and MA(2) (0.2601) terms show that errors from the previous two periods positively affect the current value, with a stronger impact from the most recent error. The model effectively balances these influences, and with low AIC (719.21) and RMSE (1.0527) values, it fits the data well.
...
As a result, ARMA(0,2) seem to be an adequate model for this time series.
...
```{r}
# Fit the model with ARMA(1,1)
arma_11_model_seriesB <- Arima(data$Series.B, order = c(1, 0, 1))

# Evaluate the model 
tsdiag(arma_11_model_seriesB)
```
...
As a result, ARMA(1,1) seem to be an adequate model for this series.

#### B.2 Try with ARMA(1,2) model

```{r}
# Fit the model with ARMA(1,2)
arma_12_model_seriesB <- Arima(data$Series.B, order = c(1, 0, 2))

# Evaluate the model 
tsdiag(arma_12_model_seriesB)
```
...
As a result, ARMA(1,2) seem to be an adequate model for this series.

#### B.3 Try with ARMA(2,1) model

```{r}
# Fit the model with ARMA(2,1)
arma_21_model_seriesB <- Arima(data$Series.B, order = c(2, 0, 1))

# Evaluate the model 
tsdiag(arma_21_model_seriesB)
```
...
As a result, ARMA(2,1) seem to be an adequate model for this series.

#### B.4 Try with ARMA(2,2) model

```{r}
# Fit the model with ARMA(2,2)
arma_22_model_seriesB <- Arima(data$Series.B, order = c(2, 0, 2))

# Evaluate the model 
tsdiag(arma_22_model_seriesB)
```
...
```{r}
# Compare the AIC from each model 
aic_arma01_seriesB <- ma_series_B$aic
aic_arma11_seriesB <- arma_11_model_seriesB$aic
aic_arma12_seriesB <- arma_12_model_seriesB$aic
aic_arma21_seriesB <- arma_21_model_seriesB$aic
aic_arma22_seriesB <- arma_22_model_seriesB$aic
...
series_B_aic_values <- data.frame(
  Model = c("ARMA(0,1)","ARMA(1,1)", "ARMA(1,2)", "ARMA(2,1)", "ARMA(2,2)", "Auto_Arima(0,2)"),
  AIC = c(aic_arma01_seriesB,aic_arma11_seriesB, aic_arma12_seriesB, aic_arma21_seriesB,
           aic_arma22_seriesB, aic_auto_arima_seriesB)
)
...
Similar to Series A,, we used AIC to compared different ARIMA model. Based on the table ARIMA(0,2) have the lowest AIC which was also chosen by the Auto Arima. The ARMA(0,2) tsdiag also shown the model has captured all of the autocorrelation with the very high Ljung - Box test p-value . Therefore, we choose ARMA(0,2) as an adequate model for this time series.

In order to interpret the model more detail we extract the coefficient of the ARMA(0,2)
