...
#### A.1 Try with ARMA(0,8) model

```{r}
# Fit the model with AR(1)
arma_008_model_seriesA <- Arima(data$Series.A, order = c(0, 0, 8))

# Evaluate the model 
tsdiag(arma_008_model_seriesA)
```

The residuals fluctuate randomly around zero with no clear patterns or trends which is a good sign. The model appears to have captured the key features of the time series as the residuals bahave like white noise. The ACF plot shows no significant autocorrelations as all values fall within the confidence bounds. This suggest that the residuals do not exhibit autocorrelation where model has successfully captured the autocorrelations in the data.

The p-values for the Ljung-Box test are consistently above 0.05 for all lags. This confirm we fail to reject the null hypothesis that there is no autocorrelation in the residuals.

Based on the diagnostic check, the MA(8) model seems to be a good fit for Series A. Let's try plotting a PACF of the residuals to see if adding an AR component could be beneficial.

```{r}
pacf(arma_008_model_seriesA$residuals)
```

The PACF plot of the residuals for Series A shows that all partial autocorrelations are within the confidence bounds, indicating that there is no significant autocorrelation left in the residuals. This suggests that the residuals behave like white noise, meaning the model has adequately captured the autoregressive structure. Since the residuals do not exhibit significant partial autocorrelation, it suggests that adding an AR component is not necessary. The current model likely captures the underlying autoregressive process effectively, and no further AR terms need to be added.

As adding an AR component does not appear beneficial, Let's experiment ARMA(0,0) white noise model and ARMA(0,1) model and ARMA(0,2) which make model easy to interpret and see if it will still lead to a good fit.

#### A.2 Try with ARMA(0,1) model
...
The residuals are centred around zero with no visible patterns, look like the good sign of the model fit well. However, there is a noticeable spike on the ACF at lags 1 to 5 that falls outside the confidence bounds, indicating there are some autocorrelation in the residuals where the model cannot capture. All the p-values for Ljung - Box statistic are below 0.05 indicates that we reject the null hypothesis that there is no autocorrelation in the residuals. As a result, the ARMA(0,1) model is not a good fit for this time series, as there is still significant autocorrelation in the residuals which the model cannot capture. Let's try plotting a PACF of the residuals to see if adding an AR component could be beneficial.

```{r}
pacf(arma_01_model_seriesA$residuals)
```

The PACF plot for the ARMA(0,1) model shows a significant spike at lag 2, suggesting that there might be an AR(2) component that could improve the model. This indicates potential autoregressive behavior at lag 2, which could justify adding an AR(2) component to the model. To explore this possibility, it's reasonable to fit an ARMA(2,1) model and assess whether it provides a better fit.
...
Similar to what we have discuss, auto arima also suggest the ARMA(2,2) with have the lowest AIC (717.22) emphasize that ARMA(2,2) seem to be an adequate model for this series. However, to make it more concise, we try to with different ARMA model and look for the lowest AIC.
...
We used AIC to compared different ARIMA model, based on the table ARMA(2,2) have the lowest AIC this emphasize that this model has captured all of the autocorrelation pattern and leave the white noise on the residual. Hence, it perform as an adequate model for this particular time series.
...
Similar to what we have discuss, auto arima also suggest the ARMA(0,2) with have the lowest AIC (681.22) emphasize that ARMA(0,2) seem to be an adequate model for this series. However, to make it more clear, we try to with different ARMA model and look for the lowest AIC.
...
Similar to Series A, we used AIC to compared different ARIMA model. Based on the table ARIMA(0,0,2) have the lowest AIC which was also chosen by the Auto Arima. The ARMA(0,0,2) tsdiag also shown the model has captured all of the autocorrelation with the very high Ljung - Box test p-value . However, ARMA(0,1) also shown the low AIC across different model, it just slightly higher than ARMA(0,2). Therefore, while ARMA(0,0,2) is an adequate model for this time series, we can consider use ARMA(0,1) which more easy to interpret as it perform quite similar to ARMA(0,2) according to parsimony method.
